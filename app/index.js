...
const setHeaderState = (scrolled) =>  {
    ...
    if(window.innerHeight + scrolled >= screen4.offsetTop && !buttonBuyNow.classList.contains('is-hidden')){
...
class CustomerLine{
    ...
    adding(){
        ...
        setTimeout(this.adding,randMinutes*1000);
        this.add();
    }
    add(){
        ...
        const data = new Date();
        const customer = this.createCustomer(randQuantity, `${data.getHours()}:${data.getMinutes()}`, this.currentCustomerId++);
